...
// --- MOCK DATA (Dữ liệu giả lập cho Teacher) ---
const stats = [
...
const TeacherDashboard: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (storedData) {
        const parsedData: UserAuthData = JSON.parse(storedData);
        const user = parsedData.user;
        setUserInfo({
          fullName: `${user.lastName} ${user.firstName}`,
          role: user.roleName,
          avatar: `https://ui-avatars.com/api/?name=${user.lastName}+${user.firstName}&background=0D8ABC&color=fff`
        });
      }
    } catch (error) {
        ...
        <nav className="sticky top-0 z-50 bg-white/90 dark:bg-gray-800/90 backdrop-blur-md border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 pl-4 border-l border-gray-200 dark:border-gray-700">
              ...
              <img src={userInfo.avatar} alt="Avatar" className="w-10 h-10 rounded-full border-2 border-white dark:border-gray-600 shadow-sm" />
            </div>
          </div>
        </nav>
